<div class="mx-auto max-w-6xl px-4 py-8 font-sans text-gray-800">
    ...
    {{-- Form Card --}}
    @if ($showForm)
        <div wire:key="user-form-{{ $formKey }}-{{ $userId ?? 'new' }}" class="mb-8 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <div class="mb-4 flex items-center justify-between">
                ...
                <span @class([
                    ...
                    'bg-blue-100 text-blue-700' => !$isEdit,
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-12">
                    ...
                    <div class="md:col-span-12">
                        ...
                        <div class="inline-flex items-center rounded-full bg-white p-1 shadow-sm ring-1 ring-gray-200">
                            <button type="button" wire:click="$set('is_active', true)" @class([
                                'rounded-full px-5 py-2 text-xs font-extrabold uppercase tracking-wide transition',
                                'bg-green-600 text-white shadow-sm' => $is_active === true,
                                'bg-gray-50 text-gray-600 hover:bg-gray-100' => $is_active !== true,
                            ])>
                                Ativo
                            </button>
                            ...
                            <button type="button" wire:click="$set('is_active', false)" @class([
                                'rounded-full px-5 py-2 text-xs font-extrabold uppercase tracking-wide transition',
                                'bg-red-600 text-white shadow-sm' => $is_active === false,
                                'bg-gray-50 text-gray-600 hover:bg-gray-100' => $is_active !== false,
                            ])>
                                Inativo
                            </button>
                        </div>
                    ...
                    {{-- Actions --}}
                    <div class="mt-6 flex flex-col-reverse gap-2 border-t pt-4 sm:flex-row sm:justify-end md:col-span-12">
                        ...
                        <button type="submit" @class([
                            ...
                            'bg-blue-600 hover:bg-blue-700' => !$isEdit,
        ...
        <div class="flex flex-col gap-3 border-b px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                <div class="inline-flex items-center rounded-full bg-gray-100 p-1">
                    <button type="button" wire:click="$set('filterStatus','todos')" @class([
                    ...
                    <button type="button" wire:click="$set('filterStatus','1')" @class([
                        'rounded-full px-4 py-1.5 text-xs font-extrabold uppercase transition',
                        'bg-green-600 text-white shadow-sm' => $filterStatus === '1',
                        'text-gray-600 hover:text-gray-800' => $filterStatus !== '1',
                    ])>
                        Ativos
                    </button>

                    <button type="button" wire:click="$set('filterStatus','0')" @class([
                        'rounded-full px-4 py-1.5 text-xs font-extrabold uppercase transition',
                        'bg-red-600 text-white shadow-sm' => $filterStatus === '0',
                        'text-gray-600 hover:text-gray-800' => $filterStatus !== '0',
                    ])>
                        Inativos
                    </button>
                </div>
                        ...
                        <tr class="transition hover:bg-gray-50" wire:key="user-{{ $user->id }}">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span @class([
                                    ...
                                    'bg-green-600' => (bool) $user->is_active === true,
                                    'bg-red-600' => (bool) $user->is_active === false,
                                ])>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    <button type="button" wire:click.prevent="edit({{ $user->id }})"
                                    ...
                                    <button type="button" wire:click.prevent="delete({{ $user->id }})"
                                        wire:confirm="Excluir este usuário?" wire:loading.attr="disabled" wire:target="delete"
